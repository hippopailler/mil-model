...
from model import is_torch_model, is_torch_tensor
...
class StainNormalizer:
    ...
    def fit(
        ...
        **kwargs,
        ...
        # Fit to a preset
              ...
              and arg1 in fit_presets[self.n.preset_tag]):
    ...
    def context(
        self,
        context: Union[str, "WSI", np.ndarray, "tf.Tensor", "torch.Tensor"]
    ):
    ...
    def set_context(
        ...
        if hasattr(self.n, 'set_context'):
            if isinstance(context, str):
                image = np.asarray(WSI(context, 500, 500).thumb(mpp=4))
            elif isinstance(context, WSI):
