...
from segment._cp_utils import outlines_list
...
from typing import Union, Optional, Iterable, List, TYPE_CHECKING, Callable, Tuple
from slide.wsi import WSI
from slide.backends._init_ import wsi_reader
from util._init_ import log, batch_generator, set_ignore_sigint
    ...
    *,
    ...
    Keyword arguments:
        ...
        window_size (int): Window size, in pixels, at which to segment cells.
            Not used if slide is a `WSI` object.
    ...
    if isinstance(slide, str):
        ...
        slide = WSI(slide, tile_px=window_size, tile_um=tile_um, verbose=False)
    ...
    proc_fn = mp.Process if slide_backend() != 'libvips' else threading.Thread
